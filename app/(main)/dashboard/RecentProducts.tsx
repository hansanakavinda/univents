...
export function RecentProducts({ products }: RecentProductsProps) {
    ...
    const handleConfirmDelete = async () => {
        ...
        setIsDeleting(true)
        try {
            const result = await ApiClient.post('/api/products/delete', { productId: deleteTarget.id })
            if (result.ok) {
                router.refresh()
    ...
    const getFormattedPrice = (product: UserProduct) => {
    ...
    return (
        <div className="space-y-4">
            {products.map((product) => (
                ...
                >
                    ...
                    <div
                        ...
                        title={product.isApproved ? 'Approved' : 'Pending Approval'}
                    />
                    ...
                    <div className="flex-1 min-w-0 mb-3 pr-6">
                        ...
                        <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-xs text-text-dim">
                            ...
                            <span className="flex items-center gap-1.5 font-semibold text-accent bg-accent/5 px-2 py-0.5 rounded border border-accent/20">
                                ...
                                <span>{getFormattedPrice(product)}</span>
                            </span>
                        </div>
                    </div>
            ...
            <ConfirmDialog
                ...
                title="Delete Item"
                description={`Are you sure you want to delete item "${deleteTarget?.title}"? This action cannot be undone.`}
                confirmLabel="Delete"
                cancelLabel="Cancel"
                variant="danger"
