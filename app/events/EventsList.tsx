        ...
        <>
            ...
            {isAuthenticated && (
                <div className="mb-6 p-4 md:p-0 flex flex-col lg:flex-row items-stretch gap-3">
                    <select
                        ...
                        className="w-full lg:w-auto min-w-[200px] h-11 px-4 text-sm rounded-xl border border-[#2d2d44] bg-[#1a1a2e] text-white focus:outline-none focus:ring-2 focus:ring-[#7c3aed] transition-all cursor-pointer appearance-none bg-[url('data:image/svg+xml;charset=US-ASCII,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20width%3D%2224%22%20height%3D%2224%22%20viewBox%3D%220%200%2024%2024%22%20fill%3D%22none%22%20stroke%3D%22%239ca3af%22%20stroke-width%3D%222%22%20stroke-linecap%3D%22round%22%20stroke-linejoin%3D%22round%22%3E%3Cpolyline%20points%3D%226%209%2012%2015%2018%209%22%3E%3C%2Fpolyline%3E%3C%2Fsvg%3E')] bg-[length:1.25rem_1.25rem] bg-[position:right_0.75rem_center] bg-no-repeat pr-10"
                    >
                    ...
                    <div className="flex h-11 rounded-xl border border-[#2d2d44] overflow-hidden bg-[#1a1a2e] lg:flex-none">
                        <button
                        ...
                        <button
                            ...
                            className={`flex-1 lg:flex-none px-4 lg:px-6 h-full flex items-center justify-center text-sm font-medium transition-colors border-l border-[#2d2d44] ${sortBy === 'happening'
                                ? 'bg-[#7c3aed] text-white'
                                : 'text-[#c4c4cc] hover:bg-[#25253d]'
                                }`}
                        >
            ...
            {isFiltering && (
                <div className="text-center py-8">
                    <div className="inline-block w-8 h-8 border-4 border-[#7c3aed] border-t-transparent rounded-full animate-spin"></div>
                    <p className="mt-2 text-sm text-[#9ca3af]">Loading events...</p>
                </div>
            )}
                        ...
                        <EventCard key={event.id} hover className='max-w-full lg:max-w-[40vw] w-full'>
                            ...
                            <article>
                                <CardContent className="p-0">
                                    <div className="p-3 md:p-6">
                                        <Link href={`/events/${event.id}`}>
                                            ...
                                            <div className="flex items-center space-x-3 mb-4">
                                                <div className="w-12 h-12 rounded-full bg-gradient-to-br from-[#7c3aed] to-[#CC5500] flex items-center justify-center text-white font-semibold text-lg flex-shrink-0">
                                                ...
                                                <div className="flex-1 min-w-0">
                                                    ...
                                                    <time dateTime={new Date(event.createdAt).toISOString()} className="text-sm text-[#9ca3af]">
                                        ...
                                        <p className={`text-[#c4c4cc] whitespace-pre-wrap leading-relaxed mb-1 text-sm md:text-base ${!expandedEvents.has(event.id) ? 'line-clamp-3' : ''}`}>{event.content}</p>
                                        {event.content.split('\n').length > 3 || event.content.length > 200 ? (
                                            <button
                                                onClick={() => toggleExpand(event.id)}
                                                className="text-[#a78bfa] text-sm font-medium hover:underline mb-4 cursor-pointer"
                                            >
                                    ...
                                    {/* Footer */}
                                    <div className="mt-4 pt-4 border-t border-[#2d2d44] flex items-center p-6">
                                        <LikeButton
                                            eventId={event.id}
            ...
            {isLoading && (
                <div className="text-center py-8">
                    <div className="inline-block w-8 h-8 border-4 border-[#7c3aed] border-t-transparent rounded-full animate-spin"></div>
                    <p className="mt-2 text-sm text-[#9ca3af]">Loading more events...</p>
                </div>
            )}
            ...
            {!hasMore && events.length > 0 && !isFiltering && isAuthenticated && (
                <div className="text-center py-8">
                    <p className="text-sm text-[#6b6b7b]">You&apos;ve reached the end! 🎉</p>
                </div>
            )}
            ...
            {!isAuthenticated && events.length > 0 && (
                <div className="text-center py-10">
                    <div className="inline-block p-6 rounded-2xl bg-gradient-to-br from-[#1a1a2e] to-[#16161a] border border-[#2d2d44]">
                        <p className="text-white font-semibold mb-2">Want to see more events?</p>
                        <p className="text-sm text-[#9ca3af] mb-4">Sign in to browse all events and create your own.</p>
                        <a
                            href="/login"
                            className="inline-flex items-center px-6 py-2.5 rounded-xl bg-[#7c3aed] text-white font-medium text-sm hover:bg-[#6d28d9] transition-colors"
                        >
                            Sign in to continue
                        </a>
                    </div>
